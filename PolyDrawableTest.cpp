...
TEST(PolyDrawableTest, Construct) {
    PolyDrawable poly(L"Jimbo");
}

TEST(PolyDrawableTest, NameGetter) {
    // Create a poly
    PolyDrawable poly(L"Jimbo");

    // Test its name
    ASSERT_EQ(poly.GetName(), L"Jimbo");
}
...
TEST(PolyDrawableTest, ColorGetterSetter) {
    ...
    // Test for default color value
    ASSERT_EQ(poly.GetColor(), *wxBLACK);

    // Set new color
    poly.SetColor(*wxRED);

    // Test for new color
    ASSERT_EQ(poly.GetColor(), *wxRED);
}
